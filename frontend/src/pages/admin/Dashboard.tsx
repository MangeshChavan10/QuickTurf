...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Don't fetch analytics if the user is not approved yet
    if (!user?.isApproved) {
      setIsLoading(false);
      return;
    }
    ...
    apiFetch("/api/admin/analytics", { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(d => {
        // Validate the shape before setting state to prevent crashes
        if (d && Array.isArray(d.monthlyTrend) && Array.isArray(d.turfBreakdown)) {
          setData(d);
        }
      })
      .catch(console.error)
      .finally(() => setIsLoading(false));
  }, [token, user?.isApproved]);

  const maxMonthly = (data && data.monthlyTrend && data.monthlyTrend.length > 0)
