    ...
    <div className="h-screen overflow-hidden bg-background flex flex-col md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto bg-background relative">
        ...
        {user?.warnings && user.warnings.length > 0 && (
          <div className="sticky top-0 z-30 p-4 sm:p-6 md:p-8 pb-0 max-w-6xl mx-auto">
            <motion.div 
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-amber-50 border-2 border-amber-200 rounded-[32px] p-6 shadow-xl shadow-amber-900/5 flex flex-col md:flex-row items-start md:items-center gap-6"
            >
              <div className="w-12 h-12 rounded-2xl bg-amber-200 flex items-center justify-center text-amber-700 shrink-0">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div className="flex-1 space-y-1">
                <h3 className="font-serif font-bold text-lg text-amber-900">Important System Notification</h3>
                <p className="text-sm text-amber-800 font-medium leading-relaxed">
                  You have received {user.warnings.length} warning{user.warnings.length > 1 ? 's' : ''} from the platform administration. Please review your recent activities to ensure compliance with our community standards.
                </p>
              </div>
              <div className="flex flex-col gap-2 w-full md:w-auto shrink-0">
                {user.warnings.slice(-1).map((w, i) => (
                  <div key={i} className="px-4 py-2 bg-white/50 rounded-xl border border-amber-200/50">
                    <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-0.5">Latest Message</p>
                    <p className="text-xs font-bold text-amber-900 line-clamp-1">"{w.message}"</p>
                  </div>
                ))}
              </div>
            </motion.div>
