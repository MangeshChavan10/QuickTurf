...
async function startServer() {
    ...
    app.get("/api/admin/turfs", authenticateAdmin, async (req, res) => {
        try {
    ...
    app.post("/api/admin/customers/:email/warn", authenticateAdmin, async (req, res) => {
        try {
            const { message } = req.body;
            if (!message)
                return res.status(400).json({ error: "Warning message required" });
    ...
    app.post("/api/admin/customers/:email/ban", authenticateAdmin, async (req, res) => {
        try {
    ...
    app.post("/api/admin/customers/:email/unban", authenticateAdmin, async (req, res) => {
        try {
            const user = await User.findOne({ email: req.params.email, role: 'user' });
            if (!user)
                return res.status(404).json({ error: "User not found" });
    ...
    app.get("/api/admin/customers/:email/detail", authenticateAdmin, async (req, res) => {
        try {
    ...
    app.post("/api/auth/send-otp", async (req, res) => {
        try {
            ...
            await Otp.findOneAndUpdate({ identifier }, { otp, expiresAt }, { upsert: true, new: true });
    ...
    app.post("/api/bookings/:id/cancel", async (req, res) => {
        try {
            ...
            const parseSlotTime = (dateStr, timeStr) => {
                ...
                if (h12) {
                    ...
                    const mn = h12[2];
                    if (h12[3].toUpperCase() === 'PM' && hr !== 12)
                        hr += 12;
                    if (h12[3].toUpperCase() === 'AM' && hr === 12)
                        hr = 0;
            ...
            if (refundAmount > 0 && rzp && booking.orderId && !booking.orderId.startsWith('SIM_')) {
                try {
                    const payments = await rzp.orders.fetchPayments(booking.orderId);
    ...
    app.post("/api/verify-payment", async (req, res) => {
        ...
        try {
            const updatedBooking = await Booking.findOneAndUpdate({ orderId }, { status: 'Confirmed' }, { new: true });
            if (!updatedBooking) {
        ...
        try {
            ...
            for (const booking of bookings) {
                ...
                if (diffHours <= 1 && diffHours > 0) {
                    ...
                    const turfName = booking.turfId?.name || 'your turf';
                    const mailOptions = {
                        from: `"QuickTurf" <${process.env.SMTP_USER}>`,
                        to: booking.userEmail,
